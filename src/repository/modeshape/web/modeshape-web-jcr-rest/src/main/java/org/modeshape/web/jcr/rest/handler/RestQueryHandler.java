...
public final class RestQueryHandler extends AbstractHandler {
    ...
    public RestQueryResult executeQuery( HttpServletRequest request,
        ...
        if (trace) {
            LOGGER.trace("restQueryResult.getRowCount(): {0}", restQueryResult.getRowCount());
            // LOGGER.trace("restQueryResult results: {0}", restQueryResult.toString());
            // try {
            //     LOGGER.trace("restQueryResult JSON: {0}", restQueryResult.toJSON());
            // } catch (org.codehaus.jettison.json.JSONException ex) {
            //     LOGGER.trace("JSONException caught: {0}", ex.toString());
            // }
        }
        ...
        for (String columnName : columnNames) {
            ...
            if (activeNode != null) {
                ...
                try {
                    ...
                    // source of NPE requires check for value != null
                    if (values != null) {
