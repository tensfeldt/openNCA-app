...
public final class RestHelper {
    ...
    public static final String CLONE_METHOD_NAME = "clone";     // WPH
    public static final String REINDEX_METHOD_NAME = "reindex";     // WPH
    ...
    // The following are used to match and return Date values.
    // The Apache commons lang FastDateFormat is used because it is thread-safe,
    // where SimpleDateFormat is not.
    private static List<Pattern> ISO8601_DATE_REGEXES = Arrays.asList(
        Pattern.compile("^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}"),
        Pattern.compile("^[0-9]{4}-[0-9]{2}-[0-9]{2}"));
    private static final List<FastDateFormat> ISO8601_DATE_PARSERS = Arrays.asList(
    ...
    public static Value jsonValueToJCRValue( Object value,
        ...
        try {
            for (int i = 0; i < ISO8601_DATE_REGEXES.size(); i++) {
                if (ISO8601_DATE_REGEXES.get(i).matcher(valueString).matches()) {
                    Date date = ISO8601_DATE_PARSERS.get(i).parse(valueString);
