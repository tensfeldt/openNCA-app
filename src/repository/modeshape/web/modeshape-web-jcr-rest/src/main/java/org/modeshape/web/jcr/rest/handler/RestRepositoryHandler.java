...
public final class RestRepositoryHandler extends AbstractHandler {
    ...
    public Response reindexRepositoryWorkspace( ServletContext context, 
                                                HttpServletRequest request, 
                                                String repositoryName,
                                                String workspaceName ) throws RepositoryException {
        logger.debug("Reindexing repository '{0}' workspace '{1}'", repositoryName, workspaceName);
        Session session = getSession(request, repositoryName, workspaceName);
        Workspace workspace = (org.modeshape.jcr.api.Workspace) session.getWorkspace();
        workspace.reindex();
        session.save();
        return Response.ok().build();
    }
