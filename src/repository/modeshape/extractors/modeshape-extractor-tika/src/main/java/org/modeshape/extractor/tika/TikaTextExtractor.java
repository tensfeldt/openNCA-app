...
public class TikaTextExtractor extends TextExtractor {
    ...
    private TextAndCSVParser tacpParser = new TextAndCSVParser();
    ...
    public void extractFrom( final Binary binary,
        ...
        processStream(binary, stream -> {
            ...
            try {
                ...
                stream.mark(0x30000000);
                parser.parse(stream, textHandler, metadata, new ParseContext());
                if (metadata.get("Content-Type") == null) {
                    ContentHandler tacpTextHandler = writeLimit == null ? new BodyContentHandler() : new BodyContentHandler(writeLimit + 1);
                    stream.reset();
                    tacpParser.parse(stream, tacpTextHandler, metadata, new ParseContext());
                    if ((metadata.get("Content-Type") != null) && metadata.get("Content-Type").contains(";")) {
                        String[] parts = metadata.get("Content-Type").split(";");
