...
public class CompactJsonWriter implements JsonWriter {
    ...
    public void write( Object object,
        ...
        } else if (object.getClass().getName().equals("org.modeshape.jcr.value.basic.NodeKeyReference")) {
            writeNodeKeyReference(object, writer);
        ...
        } else if (object instanceof BasicArray) {
            write((Iterator<?>)(((BasicArray)object).iterator()), writer);
    ...
    protected void write( String value,
        ...
        for (char c = iter.first(); c != CharacterIterator.DONE; c = iter.next()) {
            switch (c) {
                case '"':
//                case '\'':        // WPH - The JSON spec says not to escape the single quote character
    ...
    protected void write( Iterator<?> iter,
                          Writer writer ) throws IOException {
        writer.append('[');
        if (iter.hasNext()) {
            writer.append(' ');
    ...
    protected void writeNodeKeyReference( Object nodeKeyReference,
                                          Writer writer ) throws IOException {
        try {
            java.lang.reflect.Method method = nodeKeyReference.getClass().getMethod("toString");
            String s = (String) method.invoke(nodeKeyReference);
            write(s, writer);
