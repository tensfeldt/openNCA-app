...
public class RowExtractors {
    ...
    public static ExtractFromRow convert( final ExtractFromRow extractor,
        ...
        return new ExtractFromRow() {
            ...
            public Object getValueInRow( RowAccessor row ) {
                ...
                if (trace) {
                    LOGGER.trace("ExtractFromRow.getValueInRow - value : {0}", value);
                }
    ...
    public static ExtractFromRow extractUniqueKey( final int rowWidth,
                                                   final CachedNodeSupplier cache,
                                                   final Columns columns,
                                                   TypeSystem types ) {
        final TypeFactory<NodeKey> keyType = types.getNodeKeyFactory();
        final NodeKey defaultNodeKey = new NodeKey("00000000-0000-0000-0000-000000000000");
        final List<String> columnPropertyNames =  columns != null ? new ArrayList<>() : null;
        final boolean trace = LOGGER.isTraceEnabled();
        if (columnPropertyNames != null) {
            for (Column column : columns.getColumns()) {
                String name = column.getPropertyName();
                if (name.indexOf(":") > -1) {
                    columnPropertyNames.add(name.split(":")[1]);
                } else {
                    columnPropertyNames.add(name);
                }
            }
        }
        ...
        if (rowWidth == 1) {
            return new ExtractFromRow() {
                ...
                public Object getValueInRow( RowAccessor row ) {
                    if (trace) {
                        LOGGER.trace("{0}: {1}", this.toString(), NodeSequence.keyFor(row.getNode()));
                    }
        ...
        if (rowWidth == 2) {
            ...
            return new ExtractFromRow() {
                ...
                public Object getValueInRow( RowAccessor row ) {
                    if (trace) {
                        LOGGER.trace("{0}: {1}", this.toString(), Tuples.tuple(keyFor(row, 0), keyFor(row, 1)));
                    }
                    // return Tuples.tuple(NodeSequence.keyFor(row.getNode(0)), NodeSequence.keyFor(row.getNode(1)));
                    return Tuples.tuple(keyFor(row, 0), keyFor(row, 1));
                }
                ...
                private NodeKey keyFor(RowAccessor row, int column) {
                    NodeKey returnValue = NodeSequence.keyFor(row.getNode(column));
                    if (columnPropertyNames != null) {
                        CachedNode node = row.getNode(column);
                        
                        if (trace) {
                            if (node == null) {
                                LOGGER.trace("Column not found: {0}", columnPropertyNames.get(column));
                            }
                        }
                            
                        boolean propertyInColumn = false;
                        Iterator<Property> iter = node.getProperties((NodeCache) cache);
                        while (iter.hasNext()) {
                            Property property = iter.next();
                           if (columnPropertyNames.contains(property.getName().getLocalName())) {
                                propertyInColumn = true;
                                break;
                            }
                        }
                        if (!propertyInColumn) {
        ...
        if (rowWidth == 3) {
            ...
            return new ExtractFromRow() {
                ...
                public Object getValueInRow( RowAccessor row ) {
                    if (trace) {
                        LOGGER.trace("{0}: {1}", this.toString(), Tuples.tuple(keyFor(row, 0), keyFor(row, 1), keyFor(row, 2)));
                    }
                    // return Tuples.tuple(NodeSequence.keyFor(row.getNode(0)), NodeSequence.keyFor(row.getNode(1)),
                    //                     NodeSequence.keyFor(row.getNode(2)));
                    return Tuples.tuple(keyFor(row, 0), keyFor(row, 1), keyFor(row, 2));
                }
        ...
        if (rowWidth == 4) {
            ...
            return new ExtractFromRow() {
                ...
                public Object getValueInRow( RowAccessor row ) {
                    if (trace) {
                        LOGGER.trace("{0}: {1}", this.toString(), Tuples.tuple(keyFor(row, 0), keyFor(row, 1), keyFor(row, 2), keyFor(row, 3)));
                    }
                    // return Tuples.tuple(NodeSequence.keyFor(row.getNode(0)), NodeSequence.keyFor(row.getNode(1)),
                    //                     NodeSequence.keyFor(row.getNode(2)), NodeSequence.keyFor(row.getNode(3)));
                    return Tuples.tuple(keyFor(row, 0), keyFor(row, 1), keyFor(row, 2), keyFor(row, 3));
                }
        ...
        return new ExtractFromRow() {
            ...
            public Object getValueInRow( RowAccessor row ) {
                ...
                if (trace) {
                    LOGGER.trace("{0}: {1}", this.toString(), Tuples.tuple(keys));
                }
    ...
    private static String typesAsStrings(TypeSystem types) {
        StringBuilder sb = new StringBuilder();
        for (String typeName : types.getTypeNames()) {
            sb.append(typeName);
            sb.append(',');
        }
        return sb.toString();
