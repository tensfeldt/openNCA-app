    ...
    public void run() {
        ...
        try {
            ...
            if (work.getSelectedPath().equals(work.getOutputPath())) {
                ...
                } else if (parentOfOutput.getPrimaryNodeType().isNodeType("equip:dataset")
                            || parentOfOutput.getPrimaryNodeType().isNodeType("equip:dataframe")) { // WPH added condition
                    outputNode = parentOfOutput;
                    addDerivedMixin = false;
                } else {
                    if (TRACE) {
