...
public class JcrQueryResult implements org.modeshape.jcr.api.query.QueryResult {
    ...
    protected static class QueryResultRowIterator extends QueryResultIterator implements RowIterator {
        ...
        protected Value jcrParent( CachedNode node ) {
            assert node != null;
            // Every node has a parent, but we need to figure out the correct version that's exposed
            return context.createValue(PropertyType.STRING, context.getParent(node));
        }

        protected Value jcrParentPath( CachedNode node ) {
            assert node != null;
            return context.createValue(PropertyType.PATH, context.getParentPath(node));
        }
    ...
    protected static abstract class AbstractRow implements javax.jcr.query.Row {
        ...
        protected Value getValue( String columnName,
            ...
            if (PseudoColumns.contains(propertyName, true)) {
                ...
                if (PseudoColumns.isParent(propertyName)) {
                    return iterator.jcrParent(cachedNode);
                }
                if (PseudoColumns.isParentPath(propertyName)) {
                    return iterator.jcrParentPath(cachedNode);
                }
            }
