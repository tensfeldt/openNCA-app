...
import org.modeshape.jcr.query.model.NodeParent;    // WPH
import org.modeshape.jcr.query.model.NodeParentPath;    // WPH
...
import org.modeshape.jcr.query.model.ParentNode;    // WPH - Added
import org.modeshape.jcr.query.model.ParentNodeJoinCondition;   // WPH -Added
...
public class BasicSqlQueryParser implements QueryParser {
    ...
    protected JoinCondition parseJoinCondition( TokenStream tokens,
        ...
        if (tokens.canConsume("ISPARENTNODE", "(")) {
            SelectorName parent = parseSelectorName(tokens, typeSystem);
            tokens.consume(',');
            SelectorName child = parseSelectorName(tokens, typeSystem);
            tokens.consume(')');
            return parentNodeJoinCondition(child, parent);
        }
    ...
    protected Constraint parseConstraint( TokenStream tokens,
        ...
        if (tokens.canConsume("(")) {
        ...
        } else if (tokens.canConsume("ISPARENTNODE", "(")) {
            SelectorName selectorName = null;
            if (tokens.matches(ANY_VALUE, ")")) {
                if (!(source instanceof Selector)) {
                    String msg = GraphI18n.functionIsAmbiguous.text("ISPARENTNODE()", pos.getLine(), pos.getColumn());
                    throw new ParsingException(pos, msg);
                }
                selectorName = ((Selector)source).name();
            } else {
            ...
            String path = parsePath(tokens, typeSystem);
            tokens.consume(')');
            constraint = parentNode(selectorName, path);
    ...
    protected DynamicOperand parseDynamicOperand( TokenStream tokens,
        ...
        if (tokens.canConsume('(')) {
            ...
            tokens.consume(")");
        } else if (tokens.canConsume("PARENT_ID", "(")) {
            if (tokens.canConsume(")")) {
                if (source instanceof Selector) {
                    return nodeParent(((Selector)source).name());
                }
                String msg = GraphI18n.functionIsAmbiguous.text("PARENT_ID()", pos.getLine(), pos.getColumn());
                throw new ParsingException(pos, msg);
            }
            result = nodeParent(parseSelectorName(tokens, typeSystem));
            tokens.consume(")");
        } else if (tokens.canConsume("PARENT_PATH", "(")) {
            if (tokens.canConsume(")")) {
                if (source instanceof Selector) {
                    return nodeParentPath(((Selector)source).name());
                }
                String msg = GraphI18n.functionIsAmbiguous.text("PARENT_PATH()", pos.getLine(), pos.getColumn());
                throw new ParsingException(pos, msg);
            }
            result = nodeParentPath(parseSelectorName(tokens, typeSystem));
            tokens.consume(")");
    ...
    protected NodeParent nodeParent( SelectorName selector ) {
        return new NodeParent(selector);
    }

    protected NodeParentPath nodeParentPath( SelectorName selector ) {
        return new NodeParentPath(selector);
    }
    ...
    protected ParentNodeJoinCondition parentNodeJoinCondition( SelectorName child,
                                                             SelectorName parent ) {
        return new ParentNodeJoinCondition(child, parent);
    }
    ...
    protected ParentNode parentNode( SelectorName name,
                                   String path ) {
        return new ParentNode(name, path);
    }
