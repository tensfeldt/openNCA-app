...
public interface JcrQueryContext {
    ...
    /**
     * Get the parent identifier of the supplied cached node. 
     * 
     * @param node the cached node; may not be null
     * @return the parent uuid, or null if the node is not referenceable
     */
    String getParent( CachedNode node );

    /**
     * Get the parent path of the supplied cached node. 
     * 
     * @param node the cached node; may not be null
     * @return the parent path, or null if the node is not referenceable
     */
    Path getParentPath( CachedNode node );
}
