...
/**
 * A dynamic operand that evaluates to the path of a node's parent given by a selector, used in a {@link Comparison} constraint.
 */
@Immutable
public class NodeParentPath implements DynamicOperand, org.modeshape.jcr.api.query.qom.NodeParentPath {
    private static final long serialVersionUID = 1L;

    private final Set<SelectorName> selectorNames;

    /**
     * Create a dynamic operand that evaluates to the path of the node's parent identified by the selector.
     * 
     * @param selectorName the name of the selector
     * @throws IllegalArgumentException if the selector name or property name are null
     */
    public NodeParentPath( SelectorName selectorName ) {
    ...
    public boolean equals( Object obj ) {
        ...
        if (obj instanceof NodeParentPath) {
            NodeParentPath that = (NodeParentPath)obj;
