...
public class Database {
    ...
    private int determineMaxExtractedTextLength( DatabaseMetaData metaData ) {
        if (databaseTypeName.equalsIgnoreCase(ORACLEDB)) {
            return 429496728;
        }
        try {
    ...
    private void initializeStatements(DatabaseType databaseType) throws IOException {
        if (databaseTypeName == null) {
            databaseTypeName = databaseType.nameString().toLowerCase();
        }
                ...
                if (rs.next()) {
                    ...
                    if (r != null) {
                        try (CharArrayWriter caw = new CharArrayWriter()) {
                            ...
                            throw new SQLException("Got an IOException", ex);
                        }
                    }
                }
                return null;
        ...
        if (databaseTypeName.equalsIgnoreCase(ORACLEDB)) {
            try (StringReader reader = new StringReader(text)) {
                try (PreparedStatement setExtractedTextSql = prepareStatement(SET_EXTRACTED_TEXT_STMT_KEY, connection)) {
                    setExtractedTextSql.setCharacterStream(1, reader, text.length());
