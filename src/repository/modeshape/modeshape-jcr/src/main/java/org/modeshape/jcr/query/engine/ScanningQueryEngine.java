...
import org.modeshape.jcr.query.model.NodeParent;
import org.modeshape.jcr.query.model.NodeParentPath;
import org.modeshape.jcr.query.model.Not;
import org.modeshape.jcr.query.model.NullOrder;
import org.modeshape.jcr.query.model.Or;
import org.modeshape.jcr.query.model.Ordering;
import org.modeshape.jcr.query.model.ParentNode;
import org.modeshape.jcr.query.model.ParentNodeJoinCondition;
...
public class ScanningQueryEngine implements org.modeshape.jcr.query.QueryEngine {
        ...
        if (!context.getProblems().hasErrors()) {
            ...
            if (!context.getProblems().hasErrors()) {
                ...
                try {
                    ...
                    if (trace) {
                        // long count = 0;
                        // NodeSequence resultRows = results.getRows();
                        // Batch batch = resultRows.nextBatch();
                        // while (batch != null) {
                        //     while(batch.hasNext()) {
                        //         count += 1;
                        //         batch.nextRow();
                        //     }
                        //     batch = resultRows.nextBatch();
                        // }
                        // LOGGER.trace("Stopped executing query {0}: {1} - rowcount: {2}", 
                        //     context.id(), stats, count);
    ...
    protected NodeSequence createNodeSequence( QueryCommand originalQuery,
        ...
        switch (plan.getType()) {
            case ACCESS:
                if (trace) {
                    LOGGER.trace("plan.getType(): ACCESS");
                }
            ...
            case DEPENDENT_QUERY:
                if (trace) {
                    LOGGER.trace("plan.getType(): DEPENDENT_QUERY");
                }
            ...
            case DUP_REMOVE:
                if (trace) {
                    LOGGER.trace("plan.getType(): DUP_REMOVE");
                }
                assert plan.getChildCount() == 1;
                if (plan.getFirstChild().getType() == Type.SORT) {
                    ...
                    if (!rows.isEmpty() && !(rows instanceof DistinctSequence)) {
                        ...
                        rows = new DistinctSequence(rows, columns, context.getTypeSystem(), context.getBufferManager(), cache, useHeap);
                    }
                }
                break;
            case GROUP:
            ...
            case JOIN:
                if (trace) {
                    LOGGER.trace("plan.getType(): JOIN");
                }
                ...
                switch (algorithm) {
                    ...
                    case MERGE:
                        if (joinCondition instanceof SameNodeJoinCondition) {
                        ...
                        } else if (joinCondition instanceof ParentNodeJoinCondition) {
                            ParentNodeJoinCondition condition = (ParentNodeJoinCondition)joinCondition;

                            // check if the JOIN was not reversed by an optimization
                            boolean joinReversed = !leftColumns.getSelectorNames().contains(condition.getChildSelectorName());
                            ...
                            if (joinReversed) {
                                ...
                                rightExtractor = RowExtractors.extractParentNodeKey(rightIndex, cache, types);
            //CHECKSTYLE:OFF
            // System.out.println("rightExtractor is the ParentNodeKey extractor");
            //CHECKSTYLE:ON
                            } else {
                                int leftIndex = leftColumns.getSelectorIndex(condition.getChildSelectorName());
                                int rightIndex = rightColumns.getSelectorIndex(condition.getParentSelectorName());
            ...
            case LIMIT:
                if (trace) {
                    LOGGER.trace("plan.getType(): LIMIT");
                }
            ...
            case NULL:
                if (trace) {
                    LOGGER.trace("plan.getType(): NULL");
                }
                // No results ...
                rows = NodeSequence.emptySequence(columns.getColumns().size());
                break;
            case PROJECT:
                if (trace) {
                    LOGGER.trace("plan.getType(): PROJECT");
                }
            ...
            case SELECT:
                if (trace) {
                    LOGGER.trace("plan.getType(): SELECT");
                }
                ...
                if (trace) {
                    LOGGER.trace("filter - {0}", filter.toString());
                }
                rows = NodeSequence.filter(rows, filter);
                if (trace) {
                    LOGGER.trace("rows instanceof {0}", rows.getClass().getName());
                }
                break;
            case SET_OPERATION:
                if (trace) {
                    LOGGER.trace("plan.getType(): SET_OPERATION");
                }
                ...
                switch (operation) {
                    ...
                    case INTERSECT: {
                        ...
                        rows = new IntersectSequence(workspaceName, first, second, firstColumns, types, bufferManager, cache, pack, useHeap);
                        break;
                    }
                    case EXCEPT: {
            ...
            case SORT:
                if (trace) {
                    LOGGER.trace("plan.getType(): SORT");
                }
                assert plan.getChildCount() == 1;
            ...
            case SOURCE:
                if (trace) {
                    LOGGER.trace("plan.getType(): SOURCE");
                }
    ...
    protected NodeSequence createNodeSequenceForSource( QueryCommand originalQuery,
        ...
        if (index.getProviderName() == null) {
            ...
            if (pathStr != null) {
                ...
                if (IndexPlanners.PARENT_BY_PATH_INDEX_NAME.equals(name)) { // WPH - Added
                    PathFactory paths = context.getExecutionContext().getValueFactories().getPathFactory();
                    Path path = paths.create(pathStr);
                    return sources.parentNode(path, 1.0f);
                }
            }
    ...
    protected RowFilter createRowFilter( final Constraint constraint,
        ...
        if (constraint instanceof ChildNode) {
            ...
            return new RowFilter() {
                ...
                public boolean isCurrentRowValid( Batch batch ) {
                    ...
                    if (parentPath.isAncestorOf(node.getPath(cache)) && parentKey.equals(node.getParentKey(cache))) return true; // WPH
            ...
            return new RowFilter() {
                ...
                public boolean isCurrentRowValid( Batch batch ) {
                    ...
                    while (node != null) {
                        ...
                        if (ancestorPath.isAncestorOf(node.getPath(cache)) && parentKey.equals(node.getParentKey(cache))) return true; // WPH
        ...
        if (constraint instanceof ParentNode) {
            ParentNode parentConstraint = (ParentNode)constraint;
            PathFactory paths = context.getExecutionContext().getValueFactories().getPathFactory();
            final Path childPath = paths.create(parentConstraint.getChildPath());
            final NodeCache cache = context.getNodeCache(sources.getWorkspaceName());
            final CachedNode child = sources.getNodeAtPath(childPath, cache);
            if (child == null) {
                return NodeSequence.NO_PASS_ROW_FILTER;
            }
            final NodeKey childKey = child.getKey();
            final boolean isChildRoot = childPath.isRoot();
            final String selectorName = parentConstraint.getSelectorName();
            final int index = columns.getSelectorIndex(selectorName);
            return new RowFilter() {
                @Override
                public boolean isCurrentRowValid( Batch batch ) {
                    CachedNode node = batch.getNode(index);
                    if (node == null) return false;
                    if (isChildRoot) return false;
                    Path parentPath = node.getPath(cache);
                    NodeKey parentKey = node.getKey();
                    if (parentPath.isAncestorOf(childPath) && parentKey.equals(child.getParentKey(cache))) return true; // WPH
                    // Don't have to check the additional parents since we only find shared nodes in the original location ...
                    return false;
                }
    ...
    protected ExtractFromRow createExtractFromRow( DynamicOperand operand,
        ...
        if (operand instanceof NodeParent) {
            final NodeParent nodeParent = (NodeParent)operand;
            final int indexInRow = columns.getSelectorIndex(nodeParent.getSelectorName());
            final NodeCache cache = context.getNodeCache(sources.getWorkspaceName());
            final NodeKey root = cache.getRootKey();
            final TypeFactory<?> stringType = context.getTypeSystem().getStringFactory();
            return new ExtractFromRow() {
                ...
                public Object getValueInRow( RowAccessor row ) {
                    ...
                    CachedNode parentNode = cache.getNode(parentKey);
                    if (parentNode == null) return null;
                    return sources.getIdentifier(parentNode, root);
                }

                @Override
                public String toString() {
                    return "(nodeParent " + nodeParent.getSelectorName() + ")";
                }
            };
        }
        if (operand instanceof NodeParentPath) {
            //CHECKSTYLE:OFF
            System.out.println("createExtractFromRow(): (operand instanceof NodeParentPath)");
            //CHECKSTYLE:ON
            final NodeParentPath nodeParentPath = (NodeParentPath)operand;
            final int indexInRow = columns.getSelectorIndex(nodeParentPath.getSelectorName());
            final NodeCache cache = context.getNodeCache(sources.getWorkspaceName());
            if (isLike) {
                return new ExtractFromRow() {
                    @Override
                    ...
                    public Object getValueInRow( RowAccessor row ) {
                        ...
                        CachedNode parentNode = cache.getNode(parentKey);
                        if (parentNode == null) return null;
                        Path path = parentNode.getPath(cache);
                        if (path.isRoot()) {
                    ...
                    @Override
                    public String toString() {
                        return "(nodeParentPath " + nodeParentPath.getSelectorName() + ")";
                    }
                };
            }
    ...
    protected ExtractFromRow createExtractFromRow( final String selectorName,
        ...
        if (PseudoColumns.contains(propName, true)) {
            ...
            if (PseudoColumns.isParent(propName)) {
                // This is a special case of obtaining the parent identifier from the row ...
                final TypeFactory<?> typeFactory = context.getTypeSystem().getStringFactory();
                return new PropertyValueExtractor(selectorName, propertyName, typeFactory) {
                    ...
                    public Object getValueInRow( RowAccessor row ) {
                        ...
                        if (node == null) return null;
                        NodeKey parentKey = node.getParentKey(cache);
                        return parentKey != null ? parentKey.getIdentifier() : null;
                    }
                };
            }
            if (PseudoColumns.isParentPath(propName)) {
                // This is a special case of obtaining the parent identifier from the row ...
                final TypeFactory<?> typeFactory = context.getTypeSystem().getStringFactory();
                return new PropertyValueExtractor(selectorName, propertyName, typeFactory) {
                    ...
                    public Object getValueInRow( RowAccessor row ) {
                        ...
                        if (parentKey == null) return null;
                        CachedNode parentNode = cache.getNode(parentKey);
                        return parentNode != null ? parentNode.getPath(cache) : null;
                    }
                };
            }
        }
