...
public abstract class AbstractNodeKeysSequence extends BufferingSequence {
    ...
    protected AbstractNodeKeysSequence( String workspaceName,
                                        ...
                                        boolean pack,
                                        boolean useHeap ) {
        super(workspaceName, rightSequence, RowExtractors.extractUniqueKey(leftSequence.width(), nodeCache, null, types), bufferMgr, nodeCache,
              pack, useHeap, true);
     ...
     protected AbstractNodeKeysSequence( String workspaceName,
                                        NodeSequence leftSequence,
                                        NodeSequence rightSequence,
                                        Columns leftSequenceColumns,
                                        TypeSystem types,
                                        BufferManager bufferMgr,
                                        CachedNodeSupplier nodeCache,
                                        boolean pack,
                                        boolean useHeap ) {
        super(workspaceName, rightSequence, RowExtractors.extractUniqueKey(leftSequence.width(), nodeCache, leftSequenceColumns, types), bufferMgr, nodeCache,
              pack, useHeap, true);
        int leftWidth =  leftSequence.width();
        int rightWidth = rightSequence.width();
        if (leftWidth > 0 && rightWidth > 0 && leftWidth != rightWidth) {
