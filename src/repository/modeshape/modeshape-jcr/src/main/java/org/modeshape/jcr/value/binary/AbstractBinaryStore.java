...
//            }
            if (!StringUtil.isBlank(storedMimeType)
                && !storedMimeType.startsWith("text/plain")) {
                if (storedMimeType.contains(";")) {
                    String[] parts = storedMimeType.split(";");
                    storedMimeType = parts[0];
                }
                return storedMimeType;
            }
        }
        ...
        if (detectedMimeType.contains(";")) {
            String[] parts = detectedMimeType.split(";");
            detectedMimeType = parts[0];
        }
        if (binary instanceof InMemoryBinaryValue) {
