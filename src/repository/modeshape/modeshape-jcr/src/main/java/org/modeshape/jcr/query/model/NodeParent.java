...
/**
 * A dynamic operand that evaluates to the {@link Node#getParent() identifier} of a node given by a selector, used in a
 * {@link Comparison} constraint.
 */
@Immutable
public class NodeParent implements DynamicOperand, org.modeshape.jcr.api.query.qom.NodeId {
    private static final long serialVersionUID = 1L;

    private final Set<SelectorName> selectorNames;

    /**
     * Create a dynamic operand that evaluates to the {@link Node#getParent() identifier} of the node described by the
     * selector.
     * 
     * @param selectorName the name of the selector
     * @throws IllegalArgumentException if the selector name or property name are null
     */
    public NodeParent( SelectorName selectorName ) {
    ...
    public boolean equals( Object obj ) {
        ...
        if (obj instanceof NodeParent) {
            NodeParent that = (NodeParent)obj;
