...
class JcrQueryManager implements QueryManager {
    ...
    protected static class SessionQueryContext implements JcrQueryContext {
        ...
        @Override
        public String getParent( CachedNode node ) {
            if (node == null) return null;
            Node jcrNode = getNode(node);
            if (jcrNode != null) {
                try {
                    Node parentNode = jcrNode.getParent();
                    if (parentNode != null) {
                    ...
                    Logger.getLogger(getClass()).debug(e, "Error obtaining UUID from parent node");
                }
            }
            return null;
        }
        ...
        @Override
        public Path getParentPath( CachedNode node ) {
            if (node == null) return null;
            NodeKey parentKey = node.getParentKey(session.cache());
            if (parentKey == null) return null;
            CachedNode parentNode = session.cache().getNode(parentKey);
            return parentNode != null ? parentNode.getPath(session.cache()) : null;
        }
