...
public class QCRequestChecklistResource extends QCRequestBaseResource {
	/**
		...
		public Object handle(Request request, Response response) throws Exception {
			...
			try{
				...
				if (id != null) {
					...
					if (qcr != null) {
						/*asc.logAuditEntry(	"Access of QC Request Checklist", 
											qcr.getEquipId(),
											"QCRequest",
											userId,
											Props.isAudit(),
											Const.AUDIT_SUCCESS,
											1L);		*/
						AuditDetails details = asc.new AuditDetails("Access of QC Request Checklist", qcr, userId);
						asc.logAuditEntryAsync(details);

						json = marshalObject(qcr.getQcChecklistItems());
