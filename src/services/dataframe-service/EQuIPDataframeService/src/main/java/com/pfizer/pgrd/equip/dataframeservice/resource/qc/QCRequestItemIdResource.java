...
public class QCRequestItemIdResource extends QCRequestBaseResource {
	/**
		...
		public Object handle(Request request, Response response) throws Exception {
			...
			try{
				...
				if (id != null) {
					...
					QCRequestItem qci = dao.getQCRequestItem(id);
					if (qci != null) {
						/*asc.logAuditEntry(	"Access of Qc Request Item by id", 
											qci.getEquipId(),
											"QCRequestItem",
											userId,
											Props.isAudit(),
											Const.AUDIT_SUCCESS,
											1L);		*/				
						AuditDetails details = asc.new AuditDetails("Access of Qc Request Item by id", qci, userId);
						asc.logAuditEntryAsync(details);
						json = marshalObject(qci);
						response.header(HTTPHeaders.CONTENT_TYPE, HTTPContentTypes.JSON);
					} else {
						Spark.halt(HTTPStatusCodes.NOT_FOUND, "No QC Request Item with ID '" + id + "' could be found.");
					}
				} else {
					Spark.halt(HTTPStatusCodes.BAD_REQUEST, "No QC Request Item ID was provided.");
				}
			}
			catch(Exception ex){
