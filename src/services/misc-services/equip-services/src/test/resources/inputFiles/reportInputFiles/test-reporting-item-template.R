# csv-add-column.r
#
# Adds an empty named column to a csv file
#
...
for (i in 1:length(args)) {
   if (grep("=", args[i]) > 0) {
      parts <- unlist(strsplit(args[i], "="))
      params[parts[1]] <- parts[2]
   }
}

autos_data <- read.csv(file=params["INPUT1"], header=TRUE, sep=",")
outputFilename <- paste(params["OUTPUT"], "/", "OUTPUT1", sep="")
